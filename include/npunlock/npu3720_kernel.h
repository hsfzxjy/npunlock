#ifndef NPUNLOCK_NPU3720_KERNEL_H
#define NPUNLOCK_NPU3720_KERNEL_H

/*
 * Helpers for the observed NPU3720 ACT invocation ABI.
 *
 * This is a target header for MoviTools, not a host C API.  Its names encode
 * the assumptions that pointers are carried in low 32-bit fields and tensor
 * records use the observed 0x28-byte NPU3720 layout.  Those assumptions must
 * not be reused for another NPU generation without separate validation.
 */

#define ACT_ABI_MAX_RANK 15u
#define ACT_ABI_TENSOR_RECORD_BYTES 0x28u
#define ACT_ABI_DATA_PTR32_OFFSET 0x00u
#define ACT_ABI_RANK_OFFSET 0x0cu
#define ACT_ABI_DIMS_PTR32_OFFSET 0x10u

#define ACT_ABI_ALWAYS_INLINE static __attribute__((always_inline)) inline

typedef struct act_abi_invocation {
  const unsigned char *params32;
  const unsigned char *dims32;
  unsigned rank;
  unsigned element_count;
} act_abi_invocation;

ACT_ABI_ALWAYS_INLINE unsigned act_abi_load_le_u32(const unsigned char *address) {
  return (unsigned)address[0] | ((unsigned)address[1] << 8) | ((unsigned)address[2] << 16) |
         ((unsigned)address[3] << 24);
}

ACT_ABI_ALWAYS_INLINE int act_abi_load_invocation32(unsigned layer_params32,
                                                    act_abi_invocation *invocation) {
  const unsigned char *params32;
  const unsigned char *dims32;
  unsigned rank;
  unsigned count = 1;
  unsigned dimension_index;

  if (layer_params32 == 0 || invocation == 0) {
    return 0;
  }
  params32 = (const unsigned char *)layer_params32;
  rank = act_abi_load_le_u32(params32 + ACT_ABI_RANK_OFFSET);
  dims32 = (const unsigned char *)act_abi_load_le_u32(params32 + ACT_ABI_DIMS_PTR32_OFFSET);
  if (rank == 0 || rank > ACT_ABI_MAX_RANK || dims32 == 0) {
    return 0;
  }
  for (dimension_index = 0; dimension_index < rank; ++dimension_index) {
    unsigned dimension = act_abi_load_le_u32(dims32 + dimension_index * 4u);
    if (dimension == 0) {
      return 0;
    }
    count *= dimension;
  }
  invocation->params32 = params32;
  invocation->dims32 = dims32;
  invocation->rank = rank;
  invocation->element_count = count;
  return 1;
}

#define ACT_ABI_LOAD_INVOCATION32_OR_RETURN(layer_params32, invocation)                            \
  do {                                                                                             \
    if (!act_abi_load_invocation32((layer_params32), &(invocation))) {                             \
      return;                                                                                      \
    }                                                                                              \
  } while (0)

#define ACT_ABI_TENSOR_PTR32(type, invocation, record_index)                                       \
  ((type *)act_abi_load_le_u32((invocation).params32 +                                             \
                               (unsigned)(record_index) * ACT_ABI_TENSOR_RECORD_BYTES +            \
                               ACT_ABI_DATA_PTR32_OFFSET))

#define ACT_ABI_INPUT_PTR32(type, invocation, input_index)                                         \
  ACT_ABI_TENSOR_PTR32(type, invocation, input_index)

#define ACT_ABI_OUTPUT_PTR32(type, invocation, input_count)                                        \
  ACT_ABI_TENSOR_PTR32(type, invocation, input_count)

/*
 * Define the external symbols required by the observed NPU3720 mlibm.a
 * dependency closure.  strtof and __truncdfsf2 are deterministic link stubs
 * for the tested tanhf path, not general implementations.  __fixsfdi provides
 * the binary32 conversion used by that path without another compiler helper.
 * Define MLIBM_DEFINE_LINK_COMPAT to 1 before including this header in exactly
 * one translation unit that uses mlibm.a.
 */
#if defined(MLIBM_DEFINE_LINK_COMPAT) && MLIBM_DEFINE_LINK_COMPAT
float strtof(const char *text, char **end_pointer) {
  (void)text;
  (void)end_pointer;
  return 0.0f;
}

float __truncdfsf2(double value) {
  (void)value;
  return 0.0f;
}

long long __fixsfdi(float value) {
  union {
    float f;
    unsigned u;
  } bits = {value};
  unsigned sign = bits.u >> 31;
  unsigned fraction = bits.u & 0x007fffffu;
  int exponent = (int)((bits.u >> 23) & 0xffu) - 127;
  unsigned long long mantissa;
  unsigned long long integer_value;
  if (exponent < 0) {
    return 0;
  }
  mantissa = (unsigned long long)(fraction | 0x00800000u);
  integer_value = exponent <= 23 ? mantissa >> (23 - exponent) : mantissa << (exponent - 23);
  return sign ? -(long long)integer_value : (long long)integer_value;
}
#endif

#endif
